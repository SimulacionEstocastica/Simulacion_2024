...
class SimetricRandomWalkOverGraph():
    ...
    def simulate_random_walk(self, node, lengths):
        neighbors = [v for v in self.graph.neighbors(node)]
        distances = [lengths[v] for v in neighbors]
        probabilities = [dist / sum(distances) for dist in distances]
        index_choice = np.random.choice(len(neighbors), p=probabilities)
...
def CalcularEsperanza(N, graph, node):
    ...
    for n in range(N):
        ...
        while graph.node not in graph.frontera:
            lengths = graph.length_adjacent_edges(graph.node)
            rw_path = graph.simulate_random_walk(graph.node, lengths)
...
def FuncionArmonica(N):
    ...
    rw_graph = SimetricRandomWalkOverGraph(initial_node = (15, 15))

    rw_graph.graph = GridGraph(30,30)
    rw_graph.frontera = GridFrontier(30,30)
